...
def Medication():
    ...
    global get_symptoms
    get_symptoms = []
    #Method to get Symptoms from patient.
    def get():
        for i in range(0, len(variable)):
            if variable[i].get() > 0 and Symptoms[i] not in get_symptoms:
                # print(Symptoms[i],"is:",variable[i].get())
                get_symptoms.append(Symptoms[i])
        # print(get_symptoms)
        if len(get_symptoms) > 0:
            result_page()
...
def result_page():
    ...
    for i in range(0, len(get_symptoms)):
        if i > 9:
            i = i - 9
            Label(result, text=str(i + 9) + ")" + get_symptoms[i], bg='green', fg='white', font=('comic sans ms', '20'),
                  anchor="w", width=25).place(relx=0.4, rely=i * 0.1)

        i = i + 1
        Label(result, text=str(i) + ")" + get_symptoms[i - 1], bg='green', fg='white', font=('comic sans ms', '20'),
              anchor="w", width=25).place(relx=0.1, rely=i * 0.1)

    global count1, count2, count3, count4, count5, count6
    count1 = count2 = count3 = count4 = count5 = count6 = 0
    for i in get_symptoms:
    ...
    Button(lb1, text="OK", fg="white", bg="red", cursor="hand2",activebackground="red", font=('comic sans ms', '20'),
...
if __name__ == "__main__":
    Heart_Diseases = ['Coronary Artery Diseases', 'Myocardial Infarction', 'Arythmia',
                      ' Heart Defects', ' Peripheral Heart Disease', 'Hypotension']
